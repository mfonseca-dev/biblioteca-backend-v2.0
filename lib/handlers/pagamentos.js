...
const { enviarPushInterno, enviarPushParaUsuario } = require('../push');
...
// ── Resolve o valor da diária aplicando desconto de acesso especial, se houver ──
// SEMPRE calculado no servidor — nunca confia em valor vindo do cliente.
// Isso fecha a brecha que existe hoje: a isenção só era checada no frontend
// (abrirModalPagamento chamando /api/liberar-acesso direto), então uma
// chamada direta a este endpoint ignorava a isenção por completo.
async function obterValorComDesconto(usuario_id) {
  const cfgValor = await getConfig(['valor_diaria']);
  const valorBase = parseFloat((cfgValor.valor_diaria || '10').replace(',', '.'));

  const { data: especial } = await supabase
    .from('acessos_especiais')
    .select('desconto_percentual, perfil')
    .eq('usuario_id', usuario_id)
    .eq('ativo', true)
    .or(`validade.is.null,validade.gte.${new Date().toISOString().split('T')[0]}`)
    .maybeSingle();

  const descontoPercentual = especial?.desconto_percentual || 0;
  const valorFinal = Math.round(valorBase * (1 - descontoPercentual / 100) * 100) / 100;

  if (descontoPercentual > 0) {
    // Auditoria — toda aplicação de desconto/isenção fica rastreável,
    // independente de qual endpoint/fluxo chamou.
    await supabase.from('auditoria').insert({
      acao: 'desconto_acesso_especial_aplicado',
      ip: 'sistema-interno',
      detalhes: {
        usuario_id, perfil: especial?.perfil || null,
        desconto_percentual: descontoPercentual,
        valor_base: valorBase, valor_final: valorFinal
      },
      criado_em: new Date().toISOString()
    }).catch(() => {}); // auditoria nunca deve travar o pagamento
  }

  return { valorBase, descontoPercentual, valorFinal };
}
...
async function liberarCatraca(usuarioId) {
  try {
    var qsel = "controlid_person_id, nome, cpf, foto_url";
    var resBusca = await supabase.from("usuarios").select(qsel).eq("id", usuarioId).maybeSingle();
    var usuario = resBusca.data;
    if (!usuario) {
      console.warn("[liberarCatraca] usuario nao encontrado: " + usuarioId);
      return { ok: false, motivo: "Usuario nao encontrado" };
    }
    if (!usuario.controlid_person_id) {
      console.log("[liberarCatraca] sem controlid_person_id, enfileirando com foto: " + usuarioId);
    }
    ...
    const { data: fila, error } = await supabase
      ...
      .insert({
        ...
        controlid_user_id: usuario.controlid_person_id ? parseInt(usuario.controlid_person_id, 10) : 0,
    ...
    if (error) {
      if (error.code === '23505') {
        console.log(`[liberarCatraca] Colisão concorrente ao enfileirar — já está na fila`);
        return { ok: true, motivo: 'Já na fila de liberação (concorrente)' };
      }
...
if (!pag) return { ok: false, motivo: 'Pagamento não encontrado' };
  ...
  if (!usu?.ativo) {
    return { ok: false, motivo: 'Procure a recepção para regularizar sua situação.' };
  }

  // ── Verifica horário limite de acesso ────────────────────────
  const { data: cfgHora } = await supabase
    .from('configuracoes').select('valor').eq('chave', 'hora_limite_acesso').maybeSingle();
  const horaLimite = cfgHora?.valor || '19:00';
  const [hLim, mLim] = horaLimite.split(':').map(Number);
  const _fmtHoraBR = new Intl.DateTimeFormat('en-GB', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit', hour12: false });
  const _partesHora = _fmtHoraBR.formatToParts(new Date());
  const horaAtual = parseInt(_partesHora.find(p => p.type === 'hour').value, 10) * 60 + parseInt(_partesHora.find(p => p.type === 'minute').value, 10);
  const limiteMin = hLim * 60 + mLim;
  if (horaAtual >= limiteMin) {
    return { ok: false, motivo: `Acesso encerrado. A Ala não aceita novos acessos após as ${horaLimite}.` };
  }
  ...
  if (pag.status === 'confirmado') {
    ...
    const { data: acessoAtivo } = await supabase
      .from('acessos')
      .select('id, prazo_entrada_ate, entrada_fisica_confirmada_em')
      .eq('usuario_id', usuarioId)
      .eq('status', 'ativo')
      .maybeSingle();

    const acessoAtivoValido = acessoAtivo
      && (acessoAtivo.entrada_fisica_confirmada_em
          || !acessoAtivo.prazo_entrada_ate
          || new Date(acessoAtivo.prazo_entrada_ate) > new Date());

    if (acessoAtivoValido) {
      return { ok: true, motivo: 'Já confirmado e acesso ativo', reentrada: false, acesso_id: acessoAtivo.id };
    }

    // Reentrada gratuita só vale DENTRO da janela de garantia (60min após a
    // saída, ver vaga_garantida_ate). Se a garantia já expirou (ou já foi
    // processada/revogada pelo cron 'processar-garantias', que zera esse
    // campo), o pagamento antigo não dá mais direito a reentrar — precisa
    // de pagamento novo. Sem essa checagem, um único pagamento confirmado
    // liberava reentrada de graça pelo dia inteiro, mesmo após a revogação
    // automática por expiração.
    const agora = new Date().toISOString();
    const { data: ultimoEncerrado } = await supabase
      .from('acessos')
      .select('id, vaga_garantida_ate')
      .eq('usuario_id', usuarioId)
      .eq('status', 'encerrado')
    ...
    const dentroDaGarantia =
      ultimoEncerrado?.vaga_garantida_ate &&
      ultimoEncerrado.vaga_garantida_ate > agora;

    if (!dentroDaGarantia) {
      return {
        ok: false,
        motivo: 'Garantia de reentrada expirada. É necessário efetuar um novo pagamento.',
        requer_novo_pagamento: true,
      };
    }
  ...
  if (!acessoExiste) {
    const { data: novoAcesso, error: errAcesso } = await inserirAcessoAtivo({
      usuario_id: usuarioId, pagamento_id: pagamentoId, registrado_por: normalizarOrigem(origem), status: 'ativo'
    });
  if (errAcesso) {
      console.error('[processarPagamento] Erro ao inserir acesso:', errAcesso);
      return { ok: false, motivo: 'Falha ao criar acesso — pagamento mantido como pendente' };
    }
    acessoId = novoAcesso.id;
  }

// Confirma o pagamento de forma ATÔMICA — só avança se ainda estiver
  // 'pendente'. Isso evita que chamadas concorrentes (ex.: a Pagar.me manda
  // order.paid e charge.paid quase juntos para o mesmo pagamento) liberem
  // a catraca duas vezes e causem erro ao sincronizar o mesmo CPF 2x no iDSecure.
  const { data: confirmadoAgora } = await supabase
    .from('pagamentos')
  ...
  if (!confirmadoAgora) {
    console.log(`[processarPagamento] pagamento=${pagamentoId} ja confirmado por chamada concorrente, ignorando`);
    return { ok: true, acesso_id: acessoId, motivo: 'ja_confirmado_concorrente' };
  }
  ...
  // Calcula o prazo que o usuário tem para efetivamente passar pela catraca
  // (informativo por enquanto — a expiração automática real depende do
  // poller de eventos do iDFace, ainda não implementado).
  const { data: cfgPrazos } = await supabase
    .from('configuracoes')
    .select('chave, valor')
    .in('chave', ['prazo_acesso_pos_pagamento_minutos', 'tolerancia_catraca_minutos']);
  const mapaPrazos = Object.fromEntries((cfgPrazos || []).map(c => [c.chave, c.valor]));
  const prazoMinutos = parseInt(mapaPrazos.prazo_acesso_pos_pagamento_minutos || '30', 10)
                      + parseInt(mapaPrazos.tolerancia_catraca_minutos || '5', 10);
  const prazoEntradaAte = new Date(Date.now() + prazoMinutos * 60000).toISOString();

// NOVO: persiste o prazo no próprio registro de acesso
await supabase
  .from('acessos')
  .update({ prazo_entrada_ate: prazoEntradaAte })
  .eq('id', acessoId);

// NOVO: avisa o usuário via push que o pagamento foi confirmado.
// Cobre o caso da aba/app ter sido fechado ou suspenso pelo SO durante
// o pagamento externo (banco/gateway) — sem isso, a confirmação só
// aparecia se o polling em foreground continuasse rodando até o fim.
await enviarPushParaUsuario(usuarioId, {
  titulo: 'Pagamento confirmado! 🎉',
  corpo: `Seu acesso foi liberado. Você tem ${prazoMinutos} min para passar pela catraca.`,
  tipo: 'acesso',
  url: '/'
});
...
return {
  ...
  prazo_entrada_ate: prazoEntradaAte,
  prazo_entrada_minutos: prazoMinutos,
};
}
...
// ── Pagar.me ─────────────────────────────────────────────────
  if (tipo === 'pagarme') {
    try {
      ...
      if (!cpfValido(usuarioPagarme.cpf)) {
        throw new Error('CPF inválido no cadastro. Corrija seu CPF em "Meu Perfil" antes de gerar o Pix.');
      }
      const telefoneLimpo = (usuarioPagarme.telefone || '6131141027').replace(/\D/g, '');
      ...
      if (!charge || charge.status === 'failed') {
        const motivo = charge?.last_transaction?.gateway_response?.errors?.[0]?.message
          ...
          || 'Cobrança recusada pelo Pagar.me';
        throw new Error(motivo);
      }

      const pix = charge.last_transaction;
      if (!pix?.qr_code) {
        throw new Error('Pagar.me não retornou QR Code Pix');
      }
...
if (gatewayTipo === 'pagarme') {
    ...
    const resp = await fetch('https://api.pagar.me/core/v5/paymentlinks', {
      ...
      body: JSON.stringify({
       ...
       payment_settings: tipo === 'debito' ? {
          accepted_payment_methods: ['debit_card'],
          debit_card_settings: {
...
res.setHeader('Vary', 'Origin');
  ...
  // ───────────────────────────────────────────────────────────────
  // 🔍 DEBUG TEMPORÁRIO — consulta pedido direto na Pagar.me
  // ───────────────────────────────────────────────────────────────
 if (req.method === 'GET' && acao === 'debug-pagarme-order') {
    const auth = autenticado(req, res);
    if (!auth.ok) return;
    if (auth.payload.tipo !== 'admin') { return res.status(403).json({ erro: 'Apenas admin' }); }
    try {
      const { order_id, link_id } = req.query;
      if (!order_id && !link_id) return res.status(400).json({ erro: 'order_id ou link_id obrigatório' });
      const cfg = await getConfig(['gateway_chave']);
      const endpoint = link_id
        ? `https://api.pagar.me/core/v5/paymentlinks/${link_id}`
        : `https://api.pagar.me/core/v5/orders/${order_id}`;
      const resp = await fetch(endpoint, {
        headers: { 'Authorization': 'Basic ' + Buffer.from(cfg.gateway_chave + ':').toString('base64') }
      });
      const data = await resp.json();
  ...
  // ───────────────────────────────────────────────────────────────
  // 🟡 CRIAR COBRANÇA PIX — auth (app/totem, usuário logado)
  ...
  if (req.method === 'POST' && acao === 'criar-cobranca-pix') {
    ...
    try {
      ...
      if (usuario_id !== auth.payload.id && auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
        return res.status(403).json({ erro: 'Só é possível gerar cobrança para o próprio usuário' });
      }
// ── Verifica usuário bloqueado ───────────────────────────────
  const { data: usuBloq } = await supabase
    .from('usuarios').select('ativo').eq('id', usuario_id).maybeSingle();
  if (!usuBloq?.ativo) {
    return res.status(403).json({ erro: 'Procure a recepção para regularizar sua situação.' });
  }

  // ── Verifica horário limite ──────────────────────────────────
  const { data: cfgHoraLim } = await supabase
    .from('configuracoes').select('valor').eq('chave', 'hora_limite_acesso').maybeSingle();
  const _horaLimite = cfgHoraLim?.valor || '19:00';
  const [_hL, _mL] = _horaLimite.split(':').map(Number);
  const _fmtHoraBR2 = new Intl.DateTimeFormat('en-GB', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit', hour12: false });
  const _partesHora2 = _fmtHoraBR2.formatToParts(new Date());
  const _horaAtual = parseInt(_partesHora2.find(p => p.type === 'hour').value, 10) * 60 + parseInt(_partesHora2.find(p => p.type === 'minute').value, 10);
  if (_horaAtual >= (_hL * 60 + _mL)) {
    return res.status(403).json({ erro: `Acesso encerrado. A Ala não aceita novos acessos após as ${_horaLimite}.` });
  }
...
const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
      ...
      const forfeitPorSaidaManual = ultimaSaida?.motivo_saida === 'usuario'
        || (ultimaSaida?.motivo_saida === 'catraca_fisica' && !ultimaSaida?.vaga_garantida_ate);
...
const pagamentoAntesDaSaida = forfeitPorSaidaManual
     ...
     const PIX_VALIDADE_MS = 4 * 60 * 1000; // 4 min (margem de segurança sobre os 5 min do gateway)
      const pendenteAindaValido = jaExiste?.status === 'pendente'
        && jaExiste?.copia_cola
        && jaExiste?.created_at
        && (Date.now() - new Date(jaExiste.created_at).getTime()) < PIX_VALIDADE_MS;

      if (pendenteAindaValido) {
        // Retorna a cobrança existente sem criar nova (ainda dentro da validade)
        const segRestantes = 300 - Math.floor((Date.now() - new Date(jaExiste.created_at).getTime()) / 1000);
        return res.status(200).json({
          pagamento_id:  jaExiste.id,
          copia_cola:    jaExiste.copia_cola,
          qr_code:       jaExiste.qr_code,
          gateway:       'cached',
          expiracao_seg: Math.max(segRestantes, 0)
        });
      }

      // Carrega valor via getConfig (consistente com demais endpoints)
      const { valorBase, descontoPercentual, valorFinal: valor } = await obterValorComDesconto(usuario_id);

      // Cria registro pendente primeiro (para ter o ID)
      const { data: pag, error: errPag } = await supabase
        .from('pagamentos')
        .insert({ usuario_id, tipo: 'pix', valor, valor_base: valorBase, desconto_percentual: descontoPercentual, status: 'pendente', origem })
        .select('id').single();

      if (errPag) return res.status(500).json({ erro: 'Erro ao criar pagamento' });

      if (valor === 0) {
        // Isenção total — não gera cobrança no gateway, confirma direto
        const resultado = await processarPagamentoConfirmado({ pagamentoId: pag.id, usuarioId: usuario_id, origem: 'acesso_especial' });
        if (!resultado.ok) {
          return res.status(409).json({ status: 'bloqueado', mensagem: resultado.motivo, ...resultado });
        }
        return res.status(200).json({ status: 'confirmado', mensagem: 'Acesso especial — isento.', ...resultado });
      }

      // Cria cobrança no gateway (fluxo normal, valor já com desconto se houver)
      ...
      // Notifica staff urgentemente — gateway falhou, usuário pode estar na entrada aguardando
      // IMPORTANTE: usar await — em ambiente serverless, disparar sem esperar pode nunca completar
      try {
        await enviarPushInterno({
          titulo: '🚨 Falha no pagamento Pix',
          corpo: `Um usuário tentou pagar e o gateway falhou: ${e.message || 'erro desconhecido'}. Verifique a entrada.`,
          tipo: 'urgente',
          apenasStaff: true
        });
      } catch (pushErr) {
        console.error('[criar-cobranca-pix] Falha ao notificar staff:', pushErr.message);
      }
      return res.status(500).json({ erro: e.message || 'Erro ao criar cobrança Pix' });
    }
  }
  // ───────────────────────────────────────────────────────────────
  // 🔗 GERAR LINK DE PAGAMENTO — cartão no app (auth)
  // ───────────────────────────────────────────────────────────────
  if (req.method === 'POST' && acao === 'gerar-link') {
    ...
    try {
      ...
      if (!usuario_id) return res.status(400).json({ erro: 'usuario_id obrigatório' });
      if (usuario_id !== auth.payload.id && auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
        return res.status(403).json({ erro: 'Só é possível gerar link para o próprio usuário' });
      }
      ...
      const { data: pag, error: errPag } = await supabase
        .from('pagamentos')
        .insert({ usuario_id, tipo, valor, valor_base: valorBase, desconto_percentual: descontoPercentual, status: 'pendente', origem })
        .select('id').single();
      if (errPag || !pag) {
 ...
 // ───────────────────────────────────────────────────────────────
  // 🖥️ INICIAR TEF — cartão no totem (auth)
  ...
  if (req.method === 'POST' && acao === 'iniciar-tef') {
    ...
    try {
      ...
      if (!usuario_id) return res.status(400).json({ erro: 'usuario_id obrigatório' });
      if (usuario_id !== auth.payload.id && auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
        return res.status(403).json({ erro: 'Só é possível iniciar pagamento para o próprio usuário' });
      }

      const cfgValorT = await getConfig(['tef_url', 'tef_tipo']);
      const { valorBase, descontoPercentual, valorFinal: valor } = await obterValorComDesconto(usuario_id);
 ...
 // ───────────────────────────────────────────────────────────────
  // 🔍 STATUS HOJE — poll do frontend (auth)
...
if (req.method === 'GET' && acao === 'status-hoje') {
    ...
    try {
      ...
      if (!usuario_id) return res.status(400).json({ erro: 'usuario_id obrigatório' });
      if (usuario_id !== auth.payload.id && auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
        return res.status(403).json({ erro: 'Só é possível consultar status do próprio usuário' });
      }
      const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
      // Verifica se o usuário saiu voluntariamente hoje (forfeit — precisa pagar de novo)
      const { data: ultimaSaida } = await supabase
      ...
      // Busca o prazo de entrada na catraca vinculado a este pagamento,
      // pra manter o front sincronizado mesmo se o toast original foi
      // perdido (usuário fechou o app ou deu refresh antes de ver a mensagem).
      let prazoEntradaAte = null;
      let prazoEntradaMinutos = null;
      if (data.status === 'confirmado') {
        const { data: acessoVinculado } = await supabase
          .from('acessos')
          .select('prazo_entrada_ate')
          .eq('pagamento_id', data.id)
          .maybeSingle();
        if (acessoVinculado?.prazo_entrada_ate) {
          const restanteMs = new Date(acessoVinculado.prazo_entrada_ate).getTime() - Date.now();
          if (restanteMs > 0) {
            prazoEntradaAte = acessoVinculado.prazo_entrada_ate;
            prazoEntradaMinutos = Math.round(restanteMs / 60000);
          }
        }
      }
      ...
      return res.status(200).json({
        ...
        prazo_entrada_ate:     prazoEntradaAte,
        prazo_entrada_minutos: prazoEntradaMinutos
      });
    } catch (e) {
  ...
  if (req.method === 'POST' && acao === 'confirmar') {
    ...
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
      return res.status(403).json({ erro: 'Apenas staff pode confirmar pagamento manualmente' });
    }
  ...
  // ───────────────────────────────────────────────────────────────
  // ───────────────────────────────────────────────────────────────
  // ❌ CANCELAR (auth — dono do pagamento ou staff)
  // ───────────────────────────────────────────────────────────────
  if (req.method === 'POST' && acao === 'cancelar') {
    ...
    try {
      const { pagamento_id } = req.body || {};
      if (!pagamento_id) return res.status(400).json({ erro: 'pagamento_id obrigatório' });
      const { data: pag } = await supabase.from('pagamentos')
        .select('id, usuario_id, status').eq('id', pagamento_id).maybeSingle();
      if (!pag) return res.status(404).json({ erro: 'Pagamento não encontrado' });
      if (pag.usuario_id !== auth.payload.id && auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
        return res.status(403).json({ erro: 'Você só pode cancelar seu próprio pagamento' });
      }
  ...
  if (req.method === 'GET' && acao === 'pendentes') {
    ...
    if (!auth.ok) return;
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
      return res.status(403).json({ erro: 'Apenas staff pode ver pendentes' });
    }
    try {
      const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
  ...
  if (req.method === 'GET' && acao === 'pagos-hoje') {
    const auth = autenticado(req, res);
    if (!auth.ok) return;
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') { return res.status(403).json({ erro: 'Apenas admin ou recepção' }); }
    try {
      const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
      const { data } = await supabase.from('pagamentos')
  ...
  if (req.method === 'POST' && acao === 'enviar-foto') {
    ...
    try {
      ...
      if (usuario_id !== auth.payload.id && auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
        return res.status(403).json({ erro: 'Você só pode atualizar sua própria foto' });
      }
 ...
 if (req.method === 'POST' && acao === 'confirmar-direto-manual') {
    ...
    if (!auth.ok) return;
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
      return res.status(403).json({ erro: 'Acesso restrito a administradores/recepção' });
    }
    try {
      const { usuario_id, tipo = 'pix', origem = 'recepcao_manual' } = req.body || {};
      if (!usuario_id) return res.status(400).json({ erro: 'usuario_id obrigatório' });
      ...
      if (!pag) {
        ...
        const { data: novo } = await supabase.from('pagamentos')
          .insert({ usuario_id, tipo, valor, valor_base: valorBase, desconto_percentual: descontoPercentual, status: 'pendente', origem }).select('id').single();
        pag = novo;
      }
...
const resultado = await processarPagamentoConfirmado({ pagamentoId: pag.id, usuarioId: usuario_id, origem });
  ...
  if (req.method === 'POST' && acao === 'cobrar-stone') {
    ...
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') { return res.status(403).json({ erro: 'Apenas admin ou recepção' }); }
      ...
      return res.status(503).json({ erro: 'Pagamento via maquininha física ainda não disponível — aguardando integração EyeMobile.' });
    try {
   ...
   // Verifica se já tem pagamento confirmado hoje
      ...
      // Carrega valor (com desconto de acesso especial, se houver)
      const { valorBase, descontoPercentual, valorFinal: valor } = await obterValorComDesconto(usuario_id);

      // Cria registro pendente
      const { data: pag, error: errPag } = await supabase
        .from('pagamentos')
        .insert({ usuario_id, tipo, valor, valor_base: valorBase, desconto_percentual: descontoPercentual, status: 'pendente', origem: 'recepcao_stone' })
        .select('id').single();
  ...
  if (req.method === 'POST' && acao === 'utilizar-credito') {
    ...
    const totalVagas = parseInt(cfgVagas.total_vagas || '30', 10);
    const ocupadas = await contarOcupadas();
    if (ocupadas >= totalVagas) return res.status(409).json({ erro: 'Sala lotada' });
    const agora = new Date().toISOString();
  ...
  // ─────────────────────────────────────────────────────────────
  // ⏱️ PROCESSAR PRAZOS DE ENTRADA EXPIRADOS — encerra automaticamente
  //    acessos cujo prazo_entrada_ate já passou e a pessoa nunca
  //    confirmou entrada física pela catraca (entrada_fisica_confirmada_em
  //    continua nulo). Não gera crédito automaticamente — o usuário
  //    precisa reportar pelo campo Sugestões, e o admin decide via
  //    acao=gerar-credito manualmente.
  //
  //    Configuração sugerida no vercel.json, mesmo padrão do
  //    processar-garantias:
  //    {
  //      "crons": [{
  //        "path": "/api/pagamentos?acao=processar-prazos-expirados&token=SEU_CRON_TOKEN",
  //        "schedule": "*/5 * * * *"
  //      }]
  //    }
  if (
    (req.method === "POST" || req.method === "GET") &&
    acao === "processar-prazos-expirados"
  ) {
    ...
    if (!cronSecretValido && !tokenValido) {
      ...
      if (auth.payload.tipo !== "admin") {
        return res.status(403).json({ erro: "Apenas admin pode executar manualmente" });
      }
    }
    ...
    try {
      ...
      const { data: expirados, error: errBusca } = await supabase
        .from("acessos")
        .select("id, usuario_id, prazo_entrada_ate")
        .eq("status", "ativo")
        .is("entrada_fisica_confirmada_em", null)
        .not("prazo_entrada_ate", "is", null)
        .lt("prazo_entrada_ate", agora);

      if (errBusca) {
        console.error("[processar-prazos-expirados] busca:", errBusca);
        return res.status(500).json({ erro: "Erro ao buscar prazos expirados" });
      }

      if (!expirados || expirados.length === 0) {
        return res.status(200).json({ mensagem: "Nenhum prazo expirado", processados: 0 });
      }
      ...
      for (const acesso of expirados) {
        const { error: errUpdate } = await supabase
          ...
          .update({
            status: "encerrado",
            saida_em: acesso.prazo_entrada_ate,
            motivo_saida: "prazo_expirado_sem_entrada",
            vaga_garantida_ate: null,
          })
          .eq("id", acesso.id);

        if (!errUpdate) encerrados++;
      }

      console.log(`[processar-prazos-expirados] ${expirados.length} expirado(s) sem entrada fisica: ${encerrados} encerrado(s)`);
      ...
      console.error("[processar-prazos-expirados]", e);
      return res.status(500).json({ erro: "Erro interno no processamento de prazos" });
    }
  }
