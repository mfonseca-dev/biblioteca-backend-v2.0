...
try {
    const { data, error } = await supabase
      ...
      .select(`
        id, usuario_id, controlid_user_id, nome, foto_url, criado_em, acao,
    ...
    const pendentes = (data || []).map(item => ({
      ...
      acao:             item.acao || 'liberar',  // 'liberar' | 'revogar' — default preserva comportamento de registros antigos
    }));

    return res.status(200).json({ pendentes });

 } catch (err) {
    console.error('[fila] Erro completo:', JSON.stringify(err, null, 2));
