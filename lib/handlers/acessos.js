 ...
 // ─────────────────────────────────────────────────────────────
  ...
  if (req.method === 'POST' && (acao === 'encerrar' || acao === 'registrar-saida')) {
    ...
    try {
      ...
      const vagaGarantidaAte = new Date(Date.now() + travaMinutos * 60000).toISOString();
      ...
      let query = supabase
        ...
        .update({
          ...
          motivo_saida:        (auth.payload.tipo === 'admin' || auth.payload.tipo === 'recepcao') ? 'recepcao' : 'usuario',
      ...
      // Nota: a revogação do acesso facial NÃO acontece mais aqui.
      // Ela é processada pelo cron 'acao=processar-garantias', que roda
      // periodicamente e só revoga quando vaga_garantida_ate expira
      // sem uma nova entrada — respeitando a janela de retorno garantida.
