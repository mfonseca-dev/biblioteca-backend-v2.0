        ...
        if (direcao === "entrada") {
          ...
          if (pagamentoPendente) {
            const { data: acessoExistente } = await supabase
              ...
              .maybeSingle();
            pagamentoJaUsado = !!acessoExistente;
          }
  ...
  if (req.method === "POST" && acao === "evento-idsecure") {
    ...
    try {
      ...
      for (const ev of ordenados) {
        ...
        // Tenta por controlid_person_id primeiro; se não achar (ou não veio
        // user_id), cai para busca por CPF — o iDSecure já entrega o CPF
        // direto no evento, então isso deixa a identificação mais robusta
        // do que dependia só do id numérico do dispositivo.
        let usuario = null;
        if (ev.user_id) {
          const { data } = await supabase
            .from("usuarios")
            .select("id, nome, tipo")
            .eq("controlid_person_id", String(ev.user_id))
            .maybeSingle();
          usuario = data || null;
        }
        if (!usuario && cpfNormalizado) {
          const { data } = await supabase
            .from("usuarios")
            .select("id, nome, tipo")
