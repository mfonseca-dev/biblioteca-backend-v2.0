...
module.exports = async function handler(req, res) {
  ...
  // ─────────────────────────────────────────────────────────────
  // 🚪 REGISTRAR SAÍDA VOLUNTÁRIA — auth usuário (app)
  //    Chamado quando o próprio usuário clica "Sair e Liberar Vaga"
  //    no app. Encerra apenas o acesso do próprio usuário autenticado.
  // ─────────────────────────────────────────────────────────────
  if (req.method === 'POST' && acao === 'registrar-saida') {
    const auth = autenticado(req, res);
    if (!auth.ok) return;

    try {
      const { acesso_id, usuario_id } = req.body || {};
      if (!acesso_id || !usuario_id) {
        return res.status(400).json({ erro: 'acesso_id e usuario_id obrigatórios' });
      }

      // Segurança: o token autenticado deve pertencer ao mesmo usuário
      if (auth.usuario.id !== usuario_id) {
        return res.status(403).json({ erro: 'Sem permissão para encerrar este acesso' });
      }
      ...
      const { data, error } = await supabase
        ...
        .eq('id', acesso_id)
        .eq('usuario_id', usuario_id) // dupla garantia — acesso pertence ao usuário
        .is('saida_em', null)         // só encerra se ainda não saiu
        .select('id, status, saida_em')
        .single();
      ...
      console.log(`[registrar-saida] usuário ${usuario_id} encerrou acesso ${acesso_id}`);
      return res.status(200).json({ mensagem: 'Saída registrada! Vaga liberada.', acesso: data });

    } catch (e) {
      console.error('[registrar-saida]', e);
      return res.status(500).json({ erro: 'Erro interno' });
    }
  }
