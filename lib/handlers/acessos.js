  ...
  if (req.method === "GET" && acao === "meu") {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          `
          id, status, entrada_em, saida_em, registrado_por, prazo_entrada_ate,
      ...
      for (const ev of ordenados) {
        ...
        if (direcao === "entrada") {
          const { data: jaAtivo } = await supabase
            .from("acessos")
            .select("id, entrada_fisica_confirmada_em")
            .eq("usuario_id", usuario.id)
            .eq("status", "ativo")
            .maybeSingle();

          if (jaAtivo) {
            // NOVO: registra a confirmação física da entrada — o acesso já
            // existia (criado no pagamento), mas até aqui não havia prova
            // de que a pessoa de fato passou pela catraca. Sem essa marca,
            // 'ja_atualizados' não diferenciava "confirmou fisicamente" de
            // "nunca apareceu", impedindo detectar não-comparecimento.
            if (!jaAtivo.entrada_fisica_confirmada_em) {
              await supabase
                .from("acessos")
                .update({ entrada_fisica_confirmada_em: horaEvento })
                .eq("id", jaAtivo.id);
            }
          ...
          if (garantido) {
            await supabase
              ...
              .update({
                ...
                motivo_saida: null,
                entrada_fisica_confirmada_em: horaEvento, // NOVO: reentrada é sempre presença física real
              })
              .eq("id", garantido.id);
            resultado.entradas++;
            resultado.processados++;
          ...
          if (pagamentoPendente && !pagamentoJaUsado) {
            ...
            if (novoAcesso && !errInsert) {
              // NOVO: esse acesso nasceu do próprio evento físico da
              // catraca — já é, por definição, entrada confirmada.
              await supabase
                .from("acessos")
                .update({ entrada_fisica_confirmada_em: horaEvento })
                .eq("id", novoAcesso.id);
...
if (direcao === "saida") {
          ...
          enviarPushParaUsuario(usuario.id, {
            ...
            corpo: `Sua vaga está garantida até ${new Date(vagaGarantidaAte).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })}. Volte antes disso para não perdê-la.`,
            tipo: 'saida_catraca',
            url: '/'
  ...
  if (req.method === "POST" && acao === "evento-idsecure") {
    ...
    try {
      ...
      for (const ev of ordenados) {
        // Comparação tolerante a acentuação/encoding — já vimos mojibake
        // (ex: "SaÃ­da" em vez de "Saída") corromper essa string em outros
        // pontos do sistema. normalize + remoção de diacríticos garante
        // que "Entrada"/"ENTRADA"/"entrada" e "Saída"/"Saida"/"SAÍDA"/
        // variações corrompidas todas batam corretamente. Caso real:
        // 18/08/2026, eventos do Maurício caindo em "direcao_desconhecida"
        // mesmo sendo entrada/saída legítimas.
        const direcaoNormalizada = (ev.direcao || "")
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .toLowerCase();
        const direcao = direcaoNormalizada.includes("entrada")
          ? "entrada"
          : direcaoNormalizada.includes("saida")
          ? "saida"
          : "desconhecida";
...
// do campo ev.user_id do evento.
  ...
  if (req.method === "POST" && acao === "travar-vaga") {
    ...
    try {
      ...
      const { data: atualizado, error: errUpdate } = await supabase
        .from("acessos")
        .update({ vaga_garantida_ate: novaGarantia, trava_manual: true, aviso_retorno_enviado: false })
        .eq("id", acesso.id)
        .select()
        .maybeSingle();
    ...
    // ─────────────────────────────────────────────────────────────
  // 🔓 LIBERAR GARANTIA — usuário desiste da vaga garantida antes
  //    do prazo de 60min expirar. Revoga o facial na hora (mesmo
  //    padrão do 'revogarNaHora' em /encerrar) — pra acessar de novo
  //    precisa pagar novamente, não recadastrar.
  // ─────────────────────────────────────────────────────────────
  if (req.method === "POST" && acao === "liberar-garantia") {
    const auth = autenticado(req, res);
    if (!auth.ok) return;
    ...
    try {
      ...
      const { data: acesso, error: errBusca } = await supabase
        .from("acessos")
        .select("id, usuario_id, status, saida_em, vaga_garantida_ate")
        .eq("usuario_id", usuarioId)
        .eq("status", "encerrado")
        .not("vaga_garantida_ate", "is", null)
        .gt("vaga_garantida_ate", agora)
      ...
      if (errBusca) {
        console.error("[liberar-garantia] busca:", errBusca);
        return res.status(500).json({ erro: "Erro interno" });
      }
      ...
      if (errUpdate) {
        console.error("[liberar-garantia] update:", errUpdate);
        return res.status(500).json({ erro: "Erro ao liberar vaga" });
      }

      // Revoga o facial na hora — pra voltar, precisa pagar de novo
      const { data: usuarioInfo } = await supabase
        .from("usuarios")
        .select("nome, controlid_person_id")
      ...
      if (controlidUserId) {
        ...
        if (errFila)
          console.error(
            "[liberar-garantia] Erro ao enfileirar revogação:",
            errFila,
          );
      } else {
        console.warn(
          "[liberar-garantia] Revogação pulada — usuario sem controlid_person_id:",
          acesso.usuario_id,
        );
      }

      return res.status(200).json({
        mensagem: "Vaga liberada. Para acessar novamente, será necessário um novo pagamento.",
        acesso: atualizado,
      });
    } catch (e) {
      console.error("[liberar-garantia]", e);
      return res.status(500).json({ erro: "Erro interno" });
    }
  }
  // ─────────────────────────────────────────────────────────────
  ...
  if (
    ...
    try {
      const agora = new Date().toISOString();
      const { enviarPushParaUsuario } = require("../push"); // ajuste o caminho conforme a estrutura real

// ── Avisa quem está perto de perder a vaga (ainda não expirou) ──
const { data: cfgAvisoMin } = await supabase
  .from("configuracoes")
  .select("valor")
  .eq("chave", "aviso_retorno_minutos")
  .maybeSingle();
const avisoMinutos = parseInt(cfgAvisoMin?.valor || "15", 10);
const limiteAviso = new Date(Date.now() + avisoMinutos * 60000).toISOString();

const { data: prestesAExpirar, error: errAviso } = await supabase
  .from("acessos")
  .select("id, usuario_id, vaga_garantida_ate")
  .eq("status", "encerrado")
  .eq("aviso_retorno_enviado", false)
  .not("vaga_garantida_ate", "is", null)
  .gt("vaga_garantida_ate", agora)          // ainda não expirou
  .lt("vaga_garantida_ate", limiteAviso);   // mas expira em até `avisoMinutos`

if (errAviso) {
  console.error("[processar-garantias] busca aviso:", errAviso);
} else if (prestesAExpirar?.length) {
  for (const acesso of prestesAExpirar) {
    await enviarPushParaUsuario(acesso.usuario_id, {
      titulo: "Sua vaga está prestes a expirar",
      corpo: `Volte à biblioteca em até ${avisoMinutos} minutos ou sua vaga será liberada.`,
      tipo: "vaga_expirando",
      url: "/",
    });
    await supabase
      .from("acessos")
      .update({ aviso_retorno_enviado: true })
      .eq("id", acesso.id);
  }
  console.log(`[processar-garantias] ${prestesAExpirar.length} aviso(s) de retorno enviado(s)`);
}
