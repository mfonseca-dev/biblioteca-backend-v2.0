...
res.setHeader('Vary', 'Origin');
  ...
  if (req.method === 'GET' && acao === 'meu') {
    try {
      ...
      return res.status(200).json({ acesso: data || null });
    } catch (e) {
      console.error('[meu]', e);
      return res.status(500).json({ erro: 'Erro interno' });
    }
  ...
  if (req.method === 'GET' && acao === 'listar') {
    ...
    try {
      ...
      if (status === 'ocupando') {
        // "Ocupando agora" = ativo, OU encerrado mas ainda dentro da
        // janela de garantia de vaga (vaga_garantida_ate no futuro).
        const agora = new Date().toISOString();
        query = query.or(`status.eq.ativo,and(status.eq.encerrado,vaga_garantida_ate.gt.${agora})`);
      } else if (status) {
...
if (req.method === 'POST' && acao === 'registrar') {
    ...
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
      return res.status(403).json({ erro: 'Apenas staff pode registrar entrada manual' });
    }
 ...
 // ─────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────
  if (req.method === 'POST' && (acao === 'encerrar' || acao === 'registrar-saida')) {
    const auth = autenticado(req, res);
    if (!auth.ok) return;
    try {
      const { acesso_id, usuario_id, revogar_imediato } = req.body || {};
      if (!acesso_id && !usuario_id) {
        return res.status(400).json({ erro: 'acesso_id ou usuario_id obrigatório' });
      }

      const ehStaff = auth.payload.tipo === 'admin' || auth.payload.tipo === 'recepcao';
      ...
      const revogarNaHora = motivoSaida === 'usuario' || (ehStaff && revogar_imediato === true);
      ...
      if (!revogarNaHora) {
        ...
        vagaGarantidaAte = new Date(Date.now() + travaMinutos * 60000).toISOString();
      }
      ...
      if (!data) return res.status(404).json({ erro: 'Acesso ativo não encontrado' });
      ...
      if (revogarNaHora) {
        ...
        if (controlidUserId) {
          ...
          if (errFila) console.error('[encerrar] Erro ao enfileirar revogação imediata:', errFila);
        } else {
          console.warn('[encerrar] Revogação imediata pulada — usuario sem controlid_person_id:', data.usuario_id);
        }
      }
  ...
  if (req.method === 'POST' && acao === 'travar-vaga') {
    ...
    try {
      ...
      const usuarioId = (ehStaff && usuarioIdBody) ? usuarioIdBody : auth.payload.id;
      ...
      const novaGarantia = new Date(Date.now() + travaMinutos * 60000).toISOString();

      const { data: atualizado, error: errUpdate } = await supabase
        .from('acessos')
        .update({ vaga_garantida_ate: novaGarantia })
        .eq('id', acesso.id)
        .select()
        .maybeSingle();
  ...
  //    }
  // ─────────────────────────────────────────────────────────────
  if ((req.method === 'POST' || req.method === 'GET') && acao === 'processar-garantias') {
    ...
    try {
      ...
      if (errBusca) {
        ...
        return res.status(500).json({ erro: 'Erro ao buscar garantias expiradas' });
      }

      if (!expirados || expirados.length === 0) {
        return res.status(200).json({ mensagem: 'Nenhuma garantia expirada', processados: 0 });
      }
      ...
      for (const acesso of expirados) {
        ...
        if (controlidUserId) {
          ...
          if (!pendenteExistente) {
            ...
            if (errFila) {
              console.error('[processar-garantias] erro ao enfileirar revogação:', errFila);
            } else {
              revogados++;
            }
          } else {
      ...
      console.log(`[processar-garantias] ${expirados.length} expirado(s): ${revogados} enfileirado(s), ${semFacial} sem facial, ${jaNaFila} já na fila`);
      ...
      return res.status(500).json({ erro: 'Erro interno no processamento de garantias' });
    }
  }
  // ─────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────
  if ((req.method === 'POST' || req.method === 'GET') && acao === 'reset-diario') {
