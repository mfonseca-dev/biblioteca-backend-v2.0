...
module.exports = async function handler(req, res) {
  ...
  // Buscar usuário por CPF exato (PÚBLICO — totem, sem token)
    ...
    // Salvar documento pelo próprio usuário (autenticado)
  if (acao === 'salvar-documento-usuario') {
    const auth = autenticado(req, res);
    if (!auth.ok) return;
    const { usuario_id, documento_url } = req.body || {};
    if (!usuario_id || !documento_url) return res.status(400).json({ erro: 'Dados obrigatórios' });
    if (auth.usuario.id !== usuario_id && auth.tipo !== 'admin') {
      return res.status(403).json({ erro: 'Sem permissão' });
    }
    const { error } = await supabase
      .from('usuarios')
      .update({ documento_url, updated_at: new Date().toISOString() })
      .eq('id', usuario_id);
    if (error) return res.status(500).json({ erro: 'Erro ao salvar documento' });
    return res.status(200).json({ ok: true });
  }

  // Salvar documento pelo admin
  if (acao === 'salvar-documento') {
    const auth = autenticado(req, res);
    if (!auth.ok) return;
...
if (acao === 'salvar-foto') {
  ...
  if (!usuario_id || !cpf || !foto_base64) {
    return res.status(400).json({ erro: 'Dados obrigatórios' });
  }

  const cpfLimpo = cpf.replace(/\D/g, '');

  // valida vínculo do usuário
  ...
  // salva foto para o agente sincronizar depois
  const { error } = await supabase
    .from('usuarios')
    .update({
...
// Salvar documento oficial (admin)
if (acao === 'salvar-documento') {
  const auth = autenticado(req, res);
  if (!auth.ok) return;
  if (auth.tipo !== 'admin') return res.status(403).json({ erro: 'Apenas admin' });
  const { usuario_id, documento_url } = req.body || {};
  if (!usuario_id || !documento_url) return res.status(400).json({ erro: 'usuario_id e documento_url obrigatórios' });
  const { error } = await supabase
    .from('usuarios')
    .update({ documento_url, updated_at: new Date().toISOString() })
    .eq('id', usuario_id);
