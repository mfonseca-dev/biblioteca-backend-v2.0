...
res.setHeader('Vary', 'Origin');
 ...
 // ── ROTAS PUBLICAS (GET ou POST) ─────────────────────────────────────────
  ...
  if (req.method === 'GET') {
    ...
    // Listar usuários com exclusão pendente (LGPD) — sem limite de 200
    if (acao === 'listar-exclusoes-pendentes') {
      if (auth.payload.tipo !== 'admin') {
        return res.status(403).json({ erro: 'Apenas admin' });
      }
      const { data, error } = await supabase
        .from('usuarios')
        .select('id, nome, cpf, exclusao_solicitada_em, exclusao_motivo')
   ...
   // Listar usuários — restrito a staff (expõe CPF, e-mail, telefone, foto)
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
      return res.status(403).json({ erro: 'Apenas staff pode listar usuários' });
    }
  ...
  if (acao === 'cadastrar' || !acao) {
    const { nome, cpf, email, telefone, foto_base64: foto_url, tipo = 'aluno', senha, assinatura_svg } = req.body || {};
    ...
    const { data: existe } = await supabase
      .from('usuarios').select('id, nome, ativo, origem_cadastro').eq('cpf', cpfLimpo).maybeSingle();

    // CPF já existe como usuário real do app — bloqueia como sempre.
    if (existe && existe.origem_cadastro === 'app') {
    ...
    // CPF já existe só como registro bruto importado do Control iD (nunca
    // usou o app) — completa esse registro em vez de bloquear ou tentar
    // criar um segundo com o mesmo CPF (a constraint usuarios_cpf_key
    // impediria de qualquer forma). Mantém o controlid_person_id que já
    // existia, se houver — o Ciclo 4 do agente.js confere/corrige sozinho
    // depois que origem_cadastro vira 'app'.
    if (existe && existe.origem_cadastro === 'importado_controlid') {
      const updateData = { nome, email, telefone, foto_url, tipo, origem_cadastro: 'app' };
      if (assinatura_svg) {
        updateData.assinatura_svg = assinatura_svg;
        updateData.termo_aceito_em = new Date().toISOString();
      }
      if (senha) {
      ...
      const { data, error } = await supabase
        .from('usuarios').update(updateData).eq('id', existe.id)
    ...
    const insertData = { nome, cpf: cpfLimpo, email, telefone, foto_url, tipo, origem_cadastro: 'app' };
    if (assinatura_svg) {
  ...
  if (acao === 'definir-senha-primeiro-acesso') {
    ...
    const { error } = await supabase
      .from('usuarios').update({ senha_hash, origem_cadastro: 'app', updated_at: new Date().toISOString() }).eq('id', usuario_id);
  ...
  if (acao === 'redefinir-senha') {
    ...
    if (!auth.ok) return;
    if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
      return res.status(403).json({ erro: 'Apenas admin ou recepção' });
    }
    ...
    await _audit(supabase, 'redefinir_senha', _ip1, { usuario_id, por: auth.payload.tipo });
  ...
  if (acao === 'bloquear') {
    ...
    await _audit(supabase, ativo ? 'usuario_ativado' : 'usuario_bloqueado', _ip2, { usuario_id, por: auth.payload.tipo });
  ...
  if (acao === 'cancelar-exclusao') {
    ...
    await supabase.from('logs_lgpd').insert({
      ...
      usuario_id: usuario_id,
     detalhes:   `Cancelada por admin: ${auth.payload.id || 'sistema'}`,
  ...
  if (acao === 'executar-exclusao') {
    ...
    await supabase.from('logs_lgpd').insert({
      ...
      usuario_id: usuario_id,
      detalhes:   `CPF original: ${u.cpf} | Executada por: ${auth.payload.id || 'admin'}`,
      criado_em:  new Date().toISOString()
    }).catch(() => {});

    await _audit(supabase, 'exclusao_executada', new Date().toISOString(), { usuario_id, por: auth.payload.tipo });
