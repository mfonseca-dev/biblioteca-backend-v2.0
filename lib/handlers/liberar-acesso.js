...
res.setHeader('Vary', 'Origin');
  ...
  if (auth.payload.tipo !== 'admin' && auth.payload.tipo !== 'recepcao') {
    return res.status(403).json({ erro: 'Apenas staff pode liberar acesso' });
  }
  ...
  try {
    ...
    if (!existente) {
      ...
      if (errFila && errFila.code !== '23505') throw errFila;
    }
      // Enfileira com foto e controlid_user_id para o agente cadastrar no iDFace
      const { error: errFila } = await supabase
        .from('liberacoes_catraca')
    ...
    if (!acessoExiste) {
      const { error: errAcesso } = await inserirAcessoAtivo({
        usuario_id,
